...
The app is configured to automatically pull any changes that are made via a pull request to the `main` branch. To set this up make sure that GitHub has the following:

- An action variable `URL` set to the URL of the application.
- An action variable `WSGI_FILE` set to the absolute path of the file that starts the web server. When deploying using PythonAnywhere, this path should be given in the web dashboard and will look something like

```
/var/www/<username>_<region>_pythonanywhere_com_wsgi.py
```

- An action secret `SECRET_KEY` that is set to the key in the `.env` file generated earlier.
